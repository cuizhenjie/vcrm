...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  const [sent, failed, filtered, visited, delivered, funnel, roi] = await Promise.all([
    db.recipient.count({ where: { tenantId, campaignId: params.id, sendStatus: "sent" } }),
    db.recipient.count({ where: { tenantId, campaignId: params.id, sendStatus: "failed" } }),
    db.recipient.count({ where: { tenantId, campaignId: params.id, sendStatus: "filtered" } }),
    db.recipient.count({ where: { tenantId, campaignId: params.id, visited: true } }),
    db.recipient.count({ where: { tenantId, campaignId: params.id, deliveryStatus: "delivered" } }),
    conversionFunnel(params.id),
    campaignRoiOne(params.id),
  ]);
  // A/B 各变体表现：点击率 = 点击/发送，标出领先者
  const variants = await db.campaignVariant.findMany({ where: { tenantId, campaignId: params.id }, include: { template: true } });
  const variantStats = await Promise.all(variants.map(async (v) => {
    const [vsent, vvisited] = await Promise.all([
      db.recipient.count({ where: { tenantId, campaignId: params.id, variantId: v.id, sendStatus: "sent" } }),
      db.recipient.count({ where: { tenantId, campaignId: params.id, variantId: v.id, visited: true } }),
    ]);
  ...
  const [testTotal, testSent, rolloutTotal] = await Promise.all([
    db.recipient.count({ where: { tenantId, campaignId: params.id, phase: "test" } }),
    db.recipient.count({ where: { tenantId, campaignId: params.id, phase: "test", sendStatus: "sent" } }),
    db.recipient.count({ where: { tenantId, campaignId: params.id, phase: "rollout" } }),
  ]);
  const testPending = await db.recipient.count({ where: { tenantId, campaignId: params.id, phase: "test", sendStatus: "pending" } });
  // 显著性基于「测试阶段」各变体表现
  const tScores = await Promise.all((await db.campaignVariant.findMany({ where: { tenantId, campaignId: params.id } })).map(async (v) => {
    const [vs, vv] = await Promise.all([
      db.recipient.count({ where: { tenantId, campaignId: params.id, variantId: v.id, phase: "test", sendStatus: "sent" } }),
      db.recipient.count({ where: { tenantId, campaignId: params.id, variantId: v.id, phase: "test", visited: true } }),
    ]);
